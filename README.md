...
本项目是对 [`nihaixia-app`](https://github.com/jangviktor-web/nihaixia-app)（原作者 **jangviktor-web** 发布的倪海厦六经辨证 Android App）的**重写 / 移植版本**：

- 用纯静态网页（HTML / CSS / 原生 JS）重写了原 Flutter / 安卓应用，保留其六经辨证诊断逻辑、方剂 / 中药 / 针灸数据与两个实用工具（子午流注、经方剂量换算）；
- 重做了 UI 与移动端体验（宣纸水墨风、明暗主题、底部导航、表格横滑）；
- 本仓库**不含**原项目的 Flutter / Dart 源码，仅作功能与数据的参考。
